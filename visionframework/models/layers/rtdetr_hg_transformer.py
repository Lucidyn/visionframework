"""Transformer / AIFI / deformable decoder pieces for RT-DETR HG."""
...
class AIFI(TransformerEncoderLayer):
    ...
    def build_2d_sincos_position_embedding(w: int, h: int, embed_dim: int = 256, temperature: float = 10000.0):
        ...
        grid_w, grid_h = meshgrid_ij(grid_w, grid_h)
        pos_dim = embed_dim // 4
...
class MLP(nn.Module):
    def __init__(
        ...
        if residual and input_dim != output_dim:
            raise ValueError("residual requires input_dim == output_dim")
...
class MSDeformAttn(nn.Module):
    ...
    def forward(self, query, refer_bbox, value, value_shapes, value_mask=None):
        ...
        if num_points == 2:
            ...
            raise ValueError(f"refer_bbox last dim must be 2 or 4, got {num_points}")
