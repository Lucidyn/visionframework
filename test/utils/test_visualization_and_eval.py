...
def test_visualizer_draw_detections_and_results() -> None:
    """Visualizer 应能在不报错的情况下绘制检测和轨迹结果，并保持图像形状。"""
    image = _dummy_image()
...
def test_detection_evaluator_metrics() -> None:
    ...
    preds = [
        ...
        Detection(bbox=(50, 50, 60, 60), confidence=0.8, class_id=0),
    ]
...
def test_dependency_manager_and_helpers() -> None:
    ...
    - is_dependency_available 为 bool
    - import_optional_dependency 在缺失时返回 None
    - get_install_command 对已定义依赖返回字符串
    """
