...
- 添加对超分和OCR的支持（高优先级）
- 添加对自动标注的支持（高优先级）
- 添加对视觉去抖动、去模糊的支持（中优先级）
- 重构项目rodamap，建立注册表机制，配置即模型，框架只调用能力接口，实现用户自由模型管理（长期目标，低优先级）
- 添加工程化管理机制，如背压降级，端到端SLA监控，决策稳定器等（长期目标，低优先级）
- 添加对多模态的支持，不添加到主框架，作为enhance模块（长期目标，低优先级）
